...
# ---------------------------------------------------------------------------
# R30 MIDI compatibility bridge.
#
# Root cause on current master:
#   EZScore.py imports only MIDI_INSTRUMENTS from ezscore.midi, then later
#   calls build_midi_file(...) directly in the Analyse view.
#
# ezscore.midi already exports the correct build_midi_file implementation and
# its call signature matches the existing call. We expose it temporarily
# through Python builtins so the legacy monolithic EZScore.py can resolve the
# symbol without changing the MIDI engine or the synchronized timelines.
#
# This bridge is deliberately narrow and can be removed when the Analyse view
# is extracted from EZScore.py into a module that imports build_midi_file
# explicitly.
# ---------------------------------------------------------------------------
