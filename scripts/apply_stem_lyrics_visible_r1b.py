...
if head != EXPECTED_HEAD:
    raise RuntimeError(f"HEAD inattendu. Attendu={EXPECTED_HEAD} Trouvé={head}. Patch bloqué.")

source = TARGET.read_text(encoding="utf-8")

for marker in ["_LYRICS_LAYOUT_JS", "ezscore_stem_analysis_conductor_r1", "function visualXForTime(time)"]:
    if marker not in source:
        raise RuntimeError(f"Précondition absente: {marker!r}. Aucun fichier modifié.")

old = '_TEMPLATE_DIR = Path(__file__).resolve().parents[2] / "templates" / "views"\n_LYRICS_LAYOUT_JS = (_TEMPLATE_DIR / "lyrics-layout.js").read_text(encoding="utf-8")\n\n'
source = replace_once(source, old, "", "layout Paroles")

old = '_JS = _LYRICS_LAYOUT_JS + "\\n\\n" + base._PLAYER_JS\n'
new = '_JS = base._PLAYER_JS\n'
source = replace_once(source, old, new, "JS autonome")

start_marker = "conductor_js = r" + "'''" + "  // Continuous STEM conductor: one chord row + one lyric row.\n"
end_marker = "\n" + "'''" + "\n\n_JS = _replace_region(\n"
start = source.find(start_marker)
if start < 0:
    raise RuntimeError("conducteur STEM: début introuvable.")
end = source.find(end_marker, start)
if end < 0:
    raise RuntimeError("conducteur STEM: fin introuvable.")
...
with tempfile.TemporaryDirectory(prefix="ezscore_stem_lyrics_r1b_") as td:
    ...
    backup = backup_root / "stem_analysis_conductor.before_STEM_LYRICS_VISIBLE_R1b.py"
    shutil.copy2(TARGET, backup)
    shutil.copy2(candidate, TARGET)

print("EZScore_STEM_LYRICS_VISIBLE_R1b appliqué.")
