...
def backup(paths: list[Path]) -> Path:
    ...
    out = base / f"EZScore_ANALYSIS_PLAYER_R1a_backup_{stamp}"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ezscore_analysis_player_r1a_") as td:
        temp_root = Path(td)
        temp_paths = []
