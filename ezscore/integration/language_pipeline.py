...
def transcribe_original_audio_only(
    ...
    if profile.get("dominant") and profile.get("primary"):
        transcribe_kwargs["language"] = str(profile["primary"])
    ...
    stem_lab._speech_cache_path(audio_hash).write_text(
...
def install(stem_lab) -> None:
    ...
    def load_speech_versioned(audio_hash: str):
        ...
        if not _cache_is_current(dict(payload)):
            return None
        return payload
