...
"""R12 karaoke presentation layer.
...
_JS = (
  ...
  rateSelect.addEventListener("change",async() => {
    ...
    playbackRate=Math.max(.75,Math.min(1.25,Number(rateSelect.value || 1)));
    rateSelect.disabled=true;

    try {
      await ensureReady();
      await loadRateBuffers(playbackRate);
      if (wasPlaying) startSources(position);
...
_COMPONENT_R12 = st.components.v2.component(
    "ezscore_karaoke_stem_player_r12",
    html=_HTML,
    css=_CSS,
    js=_JS,
    isolate_styles=True,
...
def _component_with_r12_data(*, data: dict[str, Any], **kwargs):
    ...
    return _COMPONENT_R12(data=payload, **kwargs)


_base._COMPONENT = _component_with_r12_data
