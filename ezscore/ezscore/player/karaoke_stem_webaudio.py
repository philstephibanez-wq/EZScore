...
export default function(component) {
  ...
  function createLyricTimeline(track, sourceWords, cssClass="") {
    track.innerHTML = "";
    const nodes = sourceWords.map((w) => {
      const span = document.createElement("span");
      span.className = "lyric-token" + (cssClass ? " " + cssClass : "");
      span.textContent = w.text;
      span.style.left = (w.start * pixelsPerSecond) + "px";
      track.appendChild(span);
      return span;
    });

    // Approximate visual width to keep following words readable without
    // changing their authoritative timestamp positions.
    nodes.forEach((node, i) => {
      const current = sourceWords[i];
      const next = sourceWords[i+1];
      if (!next) return;
      const available = Math.max(8, (next.start - current.start) * pixelsPerSecond - 7);
      node.style.maxWidth = available + "px";
    });
    return nodes;
  }

  const leadNodes = createLyricTimeline(leadTrack, leadWords);
  ...
  function rebuildChordTimeline() {
    ...
    for (let i=0;i<count;i++) {
      ...
      marker.style.left = (item.start * pixelsPerSecond) + "px";
      chordTrack.appendChild(marker);
      chordNodes.push(marker);
    }
  }

  function translateTimeline(track, viewport, time) {
    if (!track || !viewport) return;
    const anchor = viewport.clientWidth * anchorRatio;
    const x = anchor - time * pixelsPerSecond;
  ...
  function renderConductor(time, force=false) {
    ...
    translateTimeline(leadTrack, leadViewport, time);
    translateTimeline(chordTrack, chordViewport, time);
    if (!backingRow.classList.contains("hidden")) {
      translateTimeline(backingTrack, backingViewport, time);
    }
