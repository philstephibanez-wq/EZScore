  ...
  <div class="ez-midi-controls">
    <label>
      ...
      <input class="ez-midi-song-volume" type="range"
             min="0" max="1.5" step="0.01" value="0.85">
    </label>
    <label>
      Volume MIDI
...
export default function(component) {
  ...
  if (!audio || !initButton || !state || !now) {
    return;
  }

  audio.src =
    "data:" + data.mime + ";base64," + data.audio_base64;
  state.textContent =
    "Synthé non chargé · Instrument : " + data.instrument_label;

  const events = Array.isArray(data.midi_events)
    ? data.midi_events
    : [];
  ...
  function loadScript(url) {
    ...
    window.__ezscoreMidiScripts[url] =
      new Promise(function(resolve, reject) {
        const selector =
          'script[data-ezscore-midi-src="' + url + '"]';
  ...
  function lowerBound(time) {
    ...
    while (low < high) {
      ...
      if (Number(events[middle].time) < time) {
        low = middle + 1;
      } else {
        high = middle;
      }
  ...
  async function initializeSynth() {
    ...
    state.textContent =
      "Chargement du moteur FluidSynth…";
    ...
    try {
      ...
      if (
        !window.JSSynth ||
        !window.JSSynth.Synthesizer
      ) {
        throw new Error(
          "js-synthesizer n'est pas disponible"
        );
      }
      ...
      synth =
        new window.JSSynth.Synthesizer();

      synth.init(
        context.sampleRate,
        {
          initialGain:
            Number(synthVolume.value),
          reverbActive: false,
          chorusActive: false
        }
      );
  ...
  async function onPlay() {
    if (!ready) {
      audio.pause();
      state.textContent =
        "Chargez d'abord le synthé MIDI.";
      return;
    }
  ...
  songVolume.addEventListener(
    "input",
    onSongVolume
  );
  synthVolume.addEventListener(
    "input",
    onSynthVolume
  );
  ...
  return function() {
    ...
    initButton.removeEventListener(
      "click",
      onInitialize
    );
    songVolume.removeEventListener(
      "input",
      onSongVolume
    );
    synthVolume.removeEventListener(
      "input",
      onSynthVolume
    );
...
def render_editor_midi_player(
    ...
    *,
    ...
    The original MP3 is the transport clock. FluidSynth runs in the browser
    through a Streamlit Components v2 component. No OS MIDI output is needed.
    """
    if not midi_events:
        return
